...
module.exports = async ctx => {
  ...
  const production = await queries.productions.updateProduction(id, productionData)
  ...
  if (Array.isArray(data.applyTo) && data.applyTo.length > 0) {
    for (const i of data.applyTo) {
      ...
      const [warehouseData] = await queries.warehouse.getWhere({production_id: production.id, warehouse_id})
      if (!warehouseData) {
