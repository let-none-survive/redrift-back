  ...
  knex(table_name)
    ...
    .then(async data => {
      ...
      for (const raw_warehouse of data) {
        const warehouseData = await Warehouse.getWhere({
          warehouse_id: raw_warehouse.id,
        })
        const warehouse = {
          ...raw_warehouse,
          productions_amount: warehouseData.reduce(
...
exports.getWarehouse = id =>
  knex(table_name)
    ...
    .then(async data => {
      const productions = []
      const warehouse_raw_data = await Warehouse.getWhere({
