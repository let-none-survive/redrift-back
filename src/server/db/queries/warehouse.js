...
exports.addToWarehouse = data => knex(table_name).insert(data).returning('*').then(firstRow)
...
exports.getWhere = where => knex(table_name).select('warehouse.*', 'warehouses.name as warehouse_name', 'productions.name as production_name').where(where)
  .leftJoin('warehouses', 'warehouses.id', 'warehouse.warehouse_id')
  .leftJoin('productions', 'productions.id', 'warehouse.production_id')
exports.getDistributedAmount = production_id => knex(table_name)
  .select('*')
  .where({ production_id })
  .then(data => {
